...
# this is with full 50 transaction context, which may be too much for some models.
# server time out

# async def reviewer_assist_analyse(transactions: List[Dict]) -> Dict:
#     """
#     Reviewer Assist: analyse a batch of transactions, identify anomalies,
#     and produce a structured review summary for a senior accountant.
#     """
...
#     system = (
#         "You are a senior AI accounting reviewer. Analyse the provided transactions "
#         "and identify key concerns. Be concise, professional, and specific. "
...
#         f"High-risk count: {len(high_risk)}. Anomalies: {len(anomalies)}. "
#         f"High-risk transactions: {json.dumps(high_risk[:5], default=str)}"
#     )
#     raw = await _ollama_generate(prompt, system)
#     try:
#         result = json.loads(raw)
